...
export const data = new SlashCommandBuilder()
    .setName('announce')
    .setDescription('Wysyła ogłoszenie na serwer')
    ...
    .addChannelOption(option =>
        option.setName('kanal')
            .setDescription('Kanał, do którego zostanie wysłane ogłoszenie')
            .setRequired(true))
...
export async function execute(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor('#ED4245')
        .addFields(
            { name: '👤 Ogłoszenie przez', value: `${interaction.user}`, inline: true },
            { name: '📅 Data', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true }
        )
        .setFooter({ 
            text: `${interaction.guild.name} • Announce`, 
            iconURL: interaction.guild.iconURL() 
        })
        .setTimestamp();
    ...
    try {
        ...
        return interaction.reply({ 
            content: `✅ Wysłano ogłoszenie na kanał <#${channel.id}>${pingRole ? ` z pingiem ${pingRole}` : ''}.`, 
            ephemeral: true 
        });
    } catch (err) {
        console.error('Błąd przy wysyłaniu ogłoszenia:', err);
        return interaction.reply({ content: '❌ Nie udało się wysłać ogłoszenia na wybrany kanał.', ephemeral: true });
    }
}
